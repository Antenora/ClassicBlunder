...
client/proc/RPBoxHTML()
 ...
 #shell{position:absolute;left:0;top:0;width:600px;height:440px;image-rendering:pixelated}
 ...
 <div id='shell'>
  ...
  <div id='view'>
   <div id='tools'>
    ...
    <div class='sq' data-a='color' id='swatch' style='left:174px' title='text colour'><div class='bg'></div><span class='ca' id='swa'>A</span><div class='c' id='swc'></div></div>
    <div class='chip' data-a='align' id='c_align' style='left:204px;width:60px' title='alignment of the current line: click to cycle left, centre, right'><div class='bg'></div><span id='alignlbl'>LEFT</span></div>
 ...
 var CTL='mapwindow.rpoverlay', MINW=600, MINH=360;
 var Z=2, OP=0.85, G={x:0,y:0,w:1200,h:880}, B=null, collapsed=false, live=false, locked=false;
 var slots=\[{t:'',at:0},{t:'',at:0},{t:'',at:0}], slot=0, mode='wysiwyg', third=false, keep=false, sentLog=\[], snippets=\[], colors=\[], curColor='#ff5555';
 ...
 function serNode(node){
  ...
  for(var i=0;i<kids.length;i++){
   ...
   if(t==='font'){ var c=n.getAttribute('color'), sz=n.getAttribute('size'); var a=''; if(c&&okColor(c)) a+=' color='+c; if(sz&&okSize(sz)) a+=' size='+sz; out+=a?('<font'+a+'>'+inner+'</font>'):inner; continue; }
 ...
 function cleanTree(srcNode,dst){
  ...
  for(var i=0;i<kids.length;i++){
   ...
   else if(t==='font'){ el=document.createElement('font'); var c=n.getAttribute('color'), sz=n.getAttribute('size'); if(c&&okColor(c)) el.setAttribute('color',c); if(sz&&okSize(sz)) el.setAttribute('size',sz); }
 ...
 function popShow(el,w){ var r=el.getBoundingClientRect(); var ox=0, oy=0, n=el; while(n&&n!==shell){ ox+=n.offsetLeft; oy+=n.offsetTop; n=n.offsetParent; } pop.style.left=Math.max(16,Math.min(ox,(shell.offsetWidth||600)-16-w))+'px'; pop.style.top=(oy+el.offsetHeight+2)+'px'; pop.style.width=w+'px'; pop.style.display='block'; }
 ...
 function colorMenu(el){
  var base=\[{label:'your speech colour',v:me.tc},{label:'your emote colour',v:me.ec},{label:'white',v:'#eaf5ff'},{label:'crimson',v:'#e4635e'},{label:'ember',v:'#f2a33a'},{label:'gold',v:'#f2c87e'},{label:'leaf',v:'#5fe08a'},{label:'sky',v:'#7ec8f0'},{label:'violet',v:'#b58cff'},{label:'rose',v:'#f08cc8'},{label:'teal',v:'#4ec9b0'},{label:'slate',v:'#b8b8d9'}];
  ...
  var h="<div class='dr hint'><span>COLOUR</span></div><div class='swgrid'>";
  ...
  h+="</div><div class='pf'><div class='bg'></div><input type='text' class='hex' id='cust' placeholder='#HEX' maxlength='7'><input type='color' id='custp' value='"+curColor+"'><div class='pick' id='pickbtn' title='open the colour picker'><div class='bg'></div><span>PICK</span></div></div>";
 ...
 for(var si=0;si<3;si++){ (function(i){ document.getElementById('s_'+i).addEventListener('click',function(){ selectSlot(i); (mode==='source'?src:ed).focus(); }); })(si); }
 function send(){
  var g=getGame();
 ...
 if(PREVIEW){
  ...
  setTimeout(function(){ if(!live){ setGeom(16,120,1200,880,2,0.85,0,0,4000,3000,0,0); } },300);
 }
...
client/proc/RPBoxQuick(text)
	...
	rpbox_last_send = world.time + 10
	mob.checkInvisibilityBreaking()
	mob.SubmitRoleplay(RPSanitize(text), 0)

client/proc/RPBoxPreview(text, third)
	if(!mob) return
	var/body = RPSanitize(istext(text) ? text : "")
	if(third && findtext(body, "//") != 1 && findtext(body, "||") != 1) body = "//" + body
