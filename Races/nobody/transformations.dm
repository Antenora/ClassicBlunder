transformation
	nobody
		...
		void_super_saiyan //temp
			...
			mastery_boons(mob/user)
				...
				if(mastery >= 25 && asc > 2)
					...
					forceadd = 0.35 * asc
					passives = list("Flicker" = 1+(mdivP*10), "Pursuer" = 2+(mdivP*10),  "SaiyanPowerVoid" = 1+(1.5*asc), "ZenkaiPower" = 0.5+(0.25*asc))
		...
		spectral_tension // temp
			passives = list("Flicker" = 1, "Pursuer" = 2)
			speedadd = 0.35
			enduranceadd = 0.35
			offenseadd = 0.35
			defenseadd = 0.35
			...
			mastery_boons(mob/user)
				...
				if(mastery >= 25 && asc > 2)
					...
					passives = list("Flicker" = 1+(mdivP*11), "Pursuer" = 2+(mdivP*12))
