		...
		super_android
			...
			mastery_boons(mob/user)
				...
				if(locate(/obj/Skills/Buffs/SpecialBuffs/MilitaryFrames/Ripper_Mode, user.contents))
					basepassives += list("LifeSteal" = 5 * asc)
				...
				if(locate(/obj/Skills/Buffs/SpecialBuffs/MilitaryFrames/Hilbert_Effect,user.contents))
					...
					//Feel free to change them. But Deicide didn't make sens ewhen the buff itself has it. And KOS-MOS doesn't showcase Gnosis-killing power outside of activating Hilbert-Effect.
				...
				if(ChipOff > 2)
					if(ChipDef > 2) //Off + Def = Fluid Form + Like Water because people underestimate these stats SMILES.
						if(ChipOff <= ChipDef *2 && ChipDef <= ChipOff *2)
							passives["FluidForm"] = round(0.5 * ((ChipOff + ChipDef)/4.5), 0.1)
							passives["LikeWater"] = round(0.5 * ((ChipOff + ChipDef)/4.5), 0.1)
