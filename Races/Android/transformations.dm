		...
		super_android
			...
			mastery_boons(mob/user)
				...
				if(locate(/obj/Skills/Buffs/SpecialBuffs/MilitaryFrames/Hilbert_Effect,user.contents))
					basepassives += list("Deicide" = 5, "EndlessNine" = 0.2)//I haven't touched this one yet, this one's a bit tricky due to it's source material.
				if(user.InfinityModule)
					basepassives += list("ManaGeneration" = 2 * asc)
				...
				if(ChipOff > 2)
					if(ChipDef > 2) //Off + Def = Fluid Form + Like Water because people underestimate these stats SMILES.
						if(ChipOff <= ChipDef *2 && ChipDef <= ChipOff *2)
							passives["FluidForm"] = round(0.5 * ((ChipOff + ChipDef)/6), 0.1)
							passives["LikeWater"] = round(0.5 * ((ChipOff + ChipDef)/6), 0.1)
