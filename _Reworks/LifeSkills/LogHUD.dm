// Collection Log menu: quality-slot grid per category + a detail panel with withdraw-to-trade.
...
/atom/movable/shud/logbtn
	...
	var/action
	var/arg
	var/atom/movable/shud/logtext/lbl
...
client
	var/tmp
		...
		log_atx = 1
		log_aty = 1
		obj/LifeSkills/MaterialBuyer/log_buyer
		log_pan_x = 0
		log_pan_y = 0
		log_pan_mx = 0
		log_pan_my = 0
...
/proc/LogBtnIcon(w, h)
	if(w == 66 && h == 18) return 'HUD/log_btn_sm.png'
	if(w == 180 && h == 24) return 'HUD/log_btn_md.png'
	if(w == 96 && h == 24) return 'HUD/log_btn_no.png'
	if(w == 284 && h == 18) return 'HUD/log_row_btn.png'
	return null
...
client/proc/OpenLogMenu(obj/LifeSkills/MaterialBuyer/buyer = null)
	if(logmenu_open) CloseLogMenu()
	if(!mob) return
	CloseMenu()
	CloseInventory()
...
// short tab labels so all ten categories fit one row
/proc/LogShortCat(cat)
	switch(cat)
		if("Monster Parts") return "PARTS"
...
client/proc/RefreshLogTabs()
	...
	for(var/cat in log_tabbtns)
		...
		if(!b) continue
		var/sel = (cat == log_cat)
		b.icon = sel ? 'HUD/log_tab_on.png' : 'HUD/log_tab.png'
...
client/proc/RefreshLogPage()
	...
	if(log_buyer)
		LogText(18, 14, 220, 16, "<span style=\"[LS_FONT]; color:[LS_C_COST]\">BUDGET $[Commas(mob.MatSellRemaining())] TODAY</span>", 0)
	else
		LogText(18, 14, 180, 16, "<span style=\"[LS_FONT]; color:[LS_C_COST]\">[LogShortCat(log_cat)] [mapped] / [mats.len]</span>", 0)
	...
	LogBG('HUD/log_detail.png', 302, 84, 246, 0.15, 0)
	if(log_wd_class)
		LogDrawWithdraw()
		return
	...
	var/dy = 142
	LogText(316, dy, 240, 14, "<span style=\"[LS_FONT]; color:#8be9ff\">HELD - click to [log_buyer ? "sell" : "withdraw"]</span>", 0); dy += 20
	var/anyq = 0
	for(var/q = QUAL_LEGENDARY to QUAL_POOR step -1)
		...
		dy += 22
	if(!anyq)
		LogText(316, dy, 260, 14, "<span style=\"[LS_FONT_BODY]; color:[LS_C_HINT]\">none held right now</span>", 0); dy += 22
	dy += 8
	LogText(316, dy, 160, 14, "<span style=\"[LS_FONT]; color:#8be9ff\">LIFETIME</span>", 0)
	LogText(446, dy, 140, 14, "<span style=\"[LS_FONT]; color:[LS_C_COST]; text-align:right\">[Commas(M.MatLogLifetime(log_sel))]</span>", 0); dy += 24
	LogText(316, dy, 160, 14, "<span style=\"[LS_FONT]; color:#8be9ff\">BEST QUALITY</span>", 0)
	var/bq = M.MatLogBest(log_sel)
	LogText(446, dy, 140, 14, "<span style=\"[LS_FONT]; color:[bq ? QualityColor(bq) : LS_C_HINT]; text-align:right\">[bq ? QualityName(bq) : "-"]</span>", 0)
...
client/proc/LogDrawWithdraw()
	...
	LogText(316, 96, 284, 18, "<span style=\"[LS_FONT]; color:#ffffff\">[log_buyer ? "SELL" : "WITHDRAW"]</span>", 0)
	var/unit = log_buyer ? LifeSellPrice(mob, log_wd_class, log_wd_quality) : 0
	LogText(316, 118, 284, 16, "<span style=\"[LS_FONT_BODY]; color:[QualityColor(log_wd_quality)]\">[QualityName(log_wd_quality)] [d ? d.name : log_wd_class] &nbsp;-&nbsp; held [held][log_buyer ? " &nbsp;-&nbsp; $[Commas(unit)] ea" : ""]</span>", 0)
	if(log_buyer)
	...
	LogBtn("+1",  "wd_adj", 1,   454, 186, 66)
	LogBtn("+10", "wd_adj", 10,  526, 186, 66)
	// presets
	LogBtn("1",   "wd_set", 1,    310, 210, 66)
	LogBtn("10",  "wd_set", 10,   382, 210, 66)
	LogBtn("100", "wd_set", 100,  454, 210, 66)
	LogBtn("MAX", "wd_set", held, 526, 210, 66)
	// confirm / cancel
	LogBtn(log_buyer ? "SELL" : "WITHDRAW", "wd_ok", null, 310, 244, 180, 24, null, "#78eb78")
	LogBtn("CANCEL", "wd_cancel", null, 496, 244, 96, 24, null, "#ff6464")

// consumed-flag dispatch off the global client/MouseWheel chain (CharacterMenu.dm)
client/proc/LogWheelScroll(delta_y)
	if(!logmenu_open) return 0
	log_scroll += (delta_y > 0 ? -1 : 1)   // wheel up = earlier rows
...
client/proc/LogButton(action, arg)
	...
	switch(action)
		...
		if("wd_type")
			spawn()
				var/n = mob.HUDNumPrompt("ENTER AMOUNT", log_wd_amount)
				if(isnull(n) || !logmenu_open || !log_wd_class) return
				log_wd_amount = round(n)
				RefreshLogPage()   // LogDrawWithdraw clamps to 1..held
		if("wd_set")
			log_wd_amount = arg
			RefreshLogPage()
		if("wd_ok")
			if(mob && log_wd_class)
				if(log_buyer)
					if(get_dist(mob, log_buyer) > 3)
						mob << "You've wandered from the buyer."
						CloseLogMenu()
						return
					mob.DoLogSell(log_wd_class, log_wd_quality, log_wd_amount)
				else
