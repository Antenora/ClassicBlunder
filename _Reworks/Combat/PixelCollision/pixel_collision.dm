...
atom/movable
	...
	proc/ApplySkillHitbox(iconFile, d=0, scale=1, ovW=0, ovH=0, ovX=-1, ovY=-1, offX=0, offY=0)
		...
		vhb_ox += vhb_ax
		vhb_oy += vhb_ay
		pixel_x = round(16 - cell[1]/2) + vhb_ax
		pixel_y = round(16 - cell[2]/2) + vhb_ay
	...
	proc/ShowHitboxDebug()
		try
			...
			vhb_dbg.pixel_x = vhb_ox
			vhb_dbg.pixel_y = vhb_oy
		catch(var/exception/e)
...
proc/HitboxesOverlap(atom/movable/F, atom/A)
	...
	var/fl = 1 + (F.x-1)*32 + 16 + F.vhb_ox - F.vhb_w/2
	var/fb = 1 + (F.y-1)*32 + 16 + F.vhb_oy - F.vhb_h/2
	var/al
	var/ab
	var/aw
	var/ah
	...
	if(istype(M) && M.vhb_w > 0)
		flyer_vs_flyer = TRUE
		al = 1 + (M.x-1)*32 + 16 + M.vhb_ox - M.vhb_w/2
		ab = 1 + (M.y-1)*32 + 16 + M.vhb_oy - M.vhb_h/2
		aw = M.vhb_w
		ah = M.vhb_h
	else
		al = A.LowerX()
...
proc/InkOverlap(atom/movable/F, atom/A, wl, wb, wr, wt)
	...
	if(ismob(A))
		var/mob/m = A
		acx = 1 + (m.x-1)*32 //body anchored at its tile SW corner; visual bob never moves the hurtbox
		acy = 1 + (m.y-1)*32
	...
	if(!FM && !AM && !arw)
		return TRUE
	var/s = F.hb_scale || 1
	var/ftcx = 1 + (F.x-1)*32 + 16 + F.vhb_ax //icon center rides the FireOffset with the art
	var/ftcy = 1 + (F.y-1)*32 + 16 + F.vhb_ay
	if(glob.PIXEL_DEBUG)
