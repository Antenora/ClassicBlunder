...
//small
			...
			if("Flame Liberator")
				...
				return KeybladePassives
			if("Wayward Wind")
				KeybladePassives=list("PULock" = 1,"Skimming" = 1+(Boost/2), "Godspeed" = 1,"BlurringStrikes" = 0.3, "AttackSpeed" = 2+(Boost/2))
				return KeybladePassives
			if("Rainfell")
			...
			if("Sweetstack")
				...
				return KeybladePassives
			if("Two Become One")
				KeybladePassives=list("PULock" = 1, "Two Become One" = 1, "BlurringStrikes" = 0.4, "ManaGeneration" = 2+(Boost/2))
				return KeybladePassives
			if("Blind Justice")
