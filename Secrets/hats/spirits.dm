...
// place parent functions here
	...
	proc/applyPassives(mob/p)
		return list("MartialMagic" = 1, "WaterWalk" = 1)
	...
	Goetic_Virtue
		applyPassives(mob/p)
			. = list("Hellpower" = currentTier/4,  "BladeFisting" = 1, "MartialMagic" = 1, "WaterWalk" = 1)
	...
	Stellar_Constellation
		applyPassives(mob/p)
			var/pot = p.Potential
			. = list("HolyMod" = currentTier, "SpiritPower" = 0.25 + (currentTier * 0.25 + (pot/250)),  "BladeFisting" = 1, "MartialMagic" = 1, "WaterWalk" = 1)
		applySecret(mob/p)
			switch(currentTier)
				if(1)
	...
	Elven_Sanctuary
		applyPassives(mob/p)
			var/pot = p.Potential
			. = list(  "SpiritualDamage" = currentTier*1.25 + pot/25, "MartialMagic" = 1, "WaterWalk" = 1)
		applySecret(mob/p)
			switch(currentTier)
				if(1)
