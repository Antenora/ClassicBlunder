	...
	Players
		...
		proc
			...
			MovementLoop()
				...
				while(src)
					if(src.pixel_z&&(key1||key2||key3||key4)&&!src.Stasis&&!src.Launched&&!src.Stunned&&!src.Suspended&&!src.PoweringUp)
			...
			stepDiagonal()
				...
				if(dir_x)
					...
					if(dir_y)
						...
						if(!src.dir_locked&&(src.Beaming!=2||src.HasTurningCharge())&&!src.Stasis&&!src.Frozen&&!src.Launched&&!src.Stunned&&!src.Suspended&&!src.PoweringUp)
							src.dir=step_d
						...
						if(prob(src.Confused) || passive_handler.Get("Manic") ? prob(passive_handler.Get("Manic") * 5) : 0)
							dir_x = pick(DIRSX)
						var/step_d=dir_x
						if(!src.dir_locked&&src.Beaming!=2&&!src.Stasis&&!src.Frozen&&!src.Launched&&!src.Stunned&&!src.Suspended&&!src.PoweringUp)
							src.dir=step_d
						if(src.Attracted&&get_dist(src, src.AttractedTo)>=3)
							src.dir=get_dir(src, src.AttractedTo)
							step_d=src.dir
