...
client/proc/NumPromptAction(action)
	switch(action)
		...
		if("sr_up") SignReaderScroll(-3)
		if("sr_down") SignReaderScroll(3)
		if("sr_close") SignReaderClose()
...
/atom/movable/shud/srpanel
	layer = NP_LAYER
	mouse_opacity = 1
	Click(location, control, params)
		var/list/plist = params2list(params)
		if(plist["right"] == "1" && usr && usr.client)
			usr.client.SignReaderClose()
...
client
	var/tmp
		...
		list/sr_lines
		list/sr_line_objs
		atom/movable/shud/nptext/sr_counter
		sr_top = 0
		sr_atx = 1
		sr_aty = 1

client/proc/SRloc(dx, dyTop, h = 0)
	var/py = SR_H - dyTop - h
	var/axp = ((dx % 32) + 32) % 32
	var/ayp = ((py % 32) + 32) % 32
	return "[sr_atx + (dx - axp) / 32]:[axp],[sr_aty + (py - ayp) / 32]:[ayp]"

client/proc/SrAdd(atom/movable/o)
	sr_hud += o
	screen += o

client/proc/SrText(dx, dyTop, w, h, txt, lay = 0.6)
	var/atom/movable/shud/nptext/T = new
	T.layer = NP_LAYER + lay
	T.maptext_width = w
	T.maptext_height = h
	T.screen_loc = SRloc(dx, dyTop, h)
	T.maptext = txt
	SrAdd(T)
	return T

client/proc/SrButton(action, label, tcol, dx, dyTop)
	var/atom/movable/shud/npbtn/b = new
	b.icon = 'HUD/np_btn.png'
	b.action = action
	b.screen_loc = SRloc(dx, dyTop, 24)
	SrAdd(b)
	SrText(dx, dyTop + 4, 100, 16, "<center><span style=\"[NP_FONT]; color:[tcol]\">[label]</span></center>", 0.62)
...
client/proc/SignReaderOpen(atom/S)
	...
	var/txt = S.desc
	if(!length(txt))
		txt = "There is nothing written here."
	txt = replacetext(txt, "\n", "<br>")
	sr_lines = WrapDescLines(txt, SR_WRAP)
	if(!sr_lines || !sr_lines.len)
		sr_lines = list("There is nothing written here.")
	sr_top = 0
	sr_open = 1
	if(!np_open)
		NpOverlaysSuspend()
	...
	if(isnull(vw)) vw = 20
	if(isnull(vh)) vh = 15
	var/mtw = round(SR_W / 32)
	if(mtw * 32 < SR_W) mtw++
	var/mth = round(SR_H / 32)
	if(mth * 32 < SR_H) mth++
	sr_atx = max(1, round((vw - mtw) / 2) + 1)
	sr_aty = max(1, round((vh - mth) / 2) + 1)
	sr_hud = list()
	var/atom/movable/shud/srpanel/P = new
	P.icon = 'HUD/desc_panel.png'
	P.screen_loc = SRloc(0, 0, SR_H)
	SrAdd(P)
	SrText(18, 14, SR_W - 36, 18, "<span style=\"[NP_FONT]; color:#8be9ff\">[html_encode(S.name)]</span>")
	sr_line_objs = list()
	for(var/i = 1 to SR_LINES)
		sr_line_objs += SrText(18, 44 + (i - 1) * 16, SR_W - 36, 16, "")
	SrButton("sr_up", "UP", "#e6f0ff", 18, 260)
	SrButton("sr_down", "DOWN", "#e6f0ff", 124, 260)
	SrButton("sr_close", "CLOSE", "#ff6464", SR_W - 118, 260)
	sr_counter = SrText(230, 264, 90, 16, "")
	SrText(0, 286, SR_W, 12, "<center><span style=\"[NP_FONT_BODY]; color:#7a9bb5\">mouse wheel or UP and DOWN to scroll - right-click the panel or CLOSE to put it away</span></center>")
	SignReaderRefresh()

client/proc/SignReaderRefresh()
	if(!sr_open || !sr_line_objs || !sr_lines)
		return
	var/maxTop = max(0, sr_lines.len - SR_LINES)
	sr_top = clamp(sr_top, 0, maxTop)
	for(var/i = 1 to SR_LINES)
		var/atom/movable/shud/nptext/L = sr_line_objs[i]
		var/idx = sr_top + i
		L.maptext = (idx <= sr_lines.len) ? "<span style=\"[NP_FONT]; color:#ffffff\">[sr_lines[idx]]</span>" : null
	if(sr_counter)
		var/last = min(sr_lines.len, sr_top + SR_LINES)
		sr_counter.maptext = (sr_lines.len > SR_LINES) ? "<center><span style=\"[NP_FONT_BODY]; color:#7a9bb5\">[sr_top + 1]-[last] of [sr_lines.len]</span></center>" : null

client/proc/SignReaderScroll(d)
	if(!sr_open)
		return
	sr_top += d
	SignReaderRefresh()

client/proc/SignWheelScroll(delta_y)
	if(!sr_open)
		return 0
	SignReaderScroll((delta_y > 0) ? -3 : 3)
	return 1
...
client/proc/SignReaderClose()
	...
	sr_open = 0
	if(sr_hud)
		for(var/atom/movable/o in sr_hud)
			screen -= o
	sr_hud = null
	sr_line_objs = null
	sr_counter = null
