...
//EPM modifications
		...
		if(!passive_handler.Get("Piloting"))
			...
			var/CyberBP
			if(isRace(ANDROID)) //Androids get more power based off their Augments + Ascensions
				if(src.EnhanceChips>0)
					CyberBP = src.EnhanceChips * 0.01 * (1 + src.AscensionsAcquired)
					Ratio *= 1 + CyberBP
