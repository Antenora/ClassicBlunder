...
client/proc/AtomCommandList(atom/A)
	...
	if(mob.Mapper)
		...
		if((istype(A, /obj/Turfs/CustomObj1) || istype(A, /turf/CustomTurf)) && hascall(mob, "Custom_Adopt")) L += list(list("m_adopt", "Custom Adopt"))
...
client/proc/AtomPanelAction(action)
	...
	switch(action)
		...
		if("m_adopt")  AdminInvoke("Custom_Adopt", T)
