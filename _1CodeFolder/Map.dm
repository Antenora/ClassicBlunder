			...
			if(k == "doorclose")
				...
				spawn()
					...
					if(!isnull(Guess) && Guess == Password)
						DoorToggle(M, 1)
					else if(!isnull(Guess) && Guess != "" && istype(src, /obj/Turfs/Door/LazerDoor))
						M << "<font color=red><small>The lazer door burns your hand!"
						M.AddBurn(1)
		proc/DoorToggle(mob/M, opening)
			var/dur = 6
			if(icon == 'Doors.dmi')
				dur = (istype(src, /obj/Turfs/Door/Door2) || istype(src, /obj/Turfs/Door/TransparentDoor)) ? 10 : 15
			antispam = 1
			spawn(dur) antispam = null
			if(opening) Open()
			else Close()
			if(istype(src, /obj/Turfs/Door/LazerDoor) && DoorID)
